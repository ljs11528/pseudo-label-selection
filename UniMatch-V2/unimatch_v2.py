...
from util.utils import count_params, init_log, AverageMeter, get_weight, semi_loss
...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            with torch.no_grad():
                pred_u_w = model_ema(img_u_w).detach()
                softmax_pred = pred_u_w.softmax(dim=1)
                mask_u_w = softmax_pred.argmax(dim=1)
                            # 动态权重计算
                weight_u_w = get_weight(softmax_pred, ignore_mask, num_classes=cfg['nclass'])
            ...
            mask_u_w_cutmixed1, weight_u_w_cutmixed1, ignore_mask_cutmixed1 = mask_u_w.clone(), weight_u_w.clone(), ignore_mask.clone()
            mask_u_w_cutmixed2, weight_u_w_cutmixed2, ignore_mask_cutmixed2 = mask_u_w.clone(), weight_u_w.clone(), ignore_mask.clone()
            ...
            mask_u_w_cutmixed1[cutmix_box1 == 1] = mask_u_w.flip(0)[cutmix_box1 == 1]
            weight_u_w_cutmixed1[cutmix_box1 == 1] = weight_u_w.flip(0)[cutmix_box1 == 1]
            ignore_mask_cutmixed1[cutmix_box1 == 1] = ignore_mask.flip(0)[cutmix_box1 == 1]
            ...
            mask_u_w_cutmixed2[cutmix_box2 == 1] = mask_u_w.flip(0)[cutmix_box2 == 1]
            weight_u_w_cutmixed2[cutmix_box2 == 1] = weight_u_w.flip(0)[cutmix_box2 == 1]
            ignore_mask_cutmixed2[cutmix_box2 == 1] = ignore_mask.flip(0)[cutmix_box2 == 1]
            ...
            # loss_u_s1 = criterion_u(pred_u_s1, mask_u_w_cutmixed1)
            # loss_u_s1 = loss_u_s1 * (weight_u_w_cutmixed1) * (ignore_mask_cutmixed1 != 255)
            # loss_u_s1 = loss_u_s1.sum() / (ignore_mask_cutmixed1 != 255).sum().item()
            ...
            loss_u_s1 = semi_loss(pred_u_s1, mask_u_w_cutmixed1, weight_u_w_cutmixed1, ignore_mask_cutmixed1, criterion_u)
            loss_u_s2 = semi_loss(pred_u_s2, mask_u_w_cutmixed2, weight_u_w_cutmixed2, ignore_mask_cutmixed2, criterion_u)
            # new
            ...
            mask_ratio = (weight_u_w > 0).sum().item() / (ignore_mask != 255).sum()
